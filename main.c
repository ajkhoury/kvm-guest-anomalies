/*
 * Reproducer code for testing side effects of pending debug exceptions in
 * KVM guest VMs.
 *
...
#include "intel_cpuid_prot_mode_syscall.inl"
...
void print_help(const char *progname)
{
	...
	printf("  intel-cpuid-prot-mode-syscall   Test Intel CPUID prot-mode syscall bit behavior\n");
}
...
int main(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		...
		} else if (strcmp(argv[i], "intel-cpuid-prot-mode-syscall") == 0) {
			return anomaly_intel_cpuid_prot_mode_syscall();
		} else {
